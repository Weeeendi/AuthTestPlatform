        ...
        if self.stateMachine == testStatus.S_GET_DEV_SN:
            ...
            if len(tmp.get('MAC', '')) == 12:
                ...
                if auth_result:
                    ...
                    log.logger.info('设备注册出错！')
                    # 设置为错误状态
                    self.listIndex = -1
                    self.stateMachine = self.stateList[self.listIndex]
                    
                    # 发送进度条信息 - 使用固定的进度值，确保显示在当前阶段
                    self.progressBar_sinOut.emit(self.testPercentCal(), False, "设备注册失败")
                ...
                # 查询MAC失败，请重试
                self.progressBar_sinOut.emit(self.testPercentCal(), False, "设备MAC查询失败")
